        ...
        <div className="space-y-3 pt-2 sm:pt-3.5 border-t border-slate-100 mt-auto">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <button
              ...
              className={`w-8 h-8 sm:w-9 sm:h-9 rounded-lg sm:rounded-xl border flex items-center justify-center transition-colors cursor-pointer shrink-0 ${savedDeals.has(deal.id) ? 'bg-[#e6f2e8] border-[#047c1f]/30 text-[#047c1f]' : 'bg-white hover:bg-slate-50 border-slate-200 text-slate-400 hover:text-slate-700'}`}
              title="Save deal"
              disabled={isExpired}
            >
            ...
            <button
              ...
              disabled={isExpired}
              className="flex-grow h-8 sm:h-9 font-extrabold text-[10px] sm:text-[12px] rounded-lg sm:rounded-xl flex items-center justify-center gap-0.5 sm:gap-1 shadow-md shadow-[#047c1f]/10 transition-colors cursor-pointer bg-[#047c1f] hover:bg-[#036318] text-white"
            >
            ...
            {currentRoute === '#deals' && (
              <div className="space-y-8 animate-in fade-in duration-300">
                <div className="space-y-2">
                  <h2 className="text-3xl font-display font-extrabold text-slate-900">All Deals & Coupon Codes</h2>
                  <p className="text-slate-500 font-medium">Search and filter active store promotions and voucher discounts</p>
                </div>

                {/* Filter Settings and state pill rows */}
                <div className="space-y-4 bg-white p-4 border border-[#e8e8e8] rounded-3xl shadow-sm">
                  <div className="relative w-full">
                    ...
                    <input
                      ...
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      className="w-full bg-slate-50 text-sm pl-10 pr-4 py-2.5 rounded-xl border border-transparent focus:outline-none focus:ring-2 focus:ring-[#047c1f]/20"
                    />
                  </div>

                  {/* Category select row */}
                  <div className="flex items-center gap-1.5 pt-1 overflow-x-auto no-scrollbar flex-nowrap w-full">
                    <span className="text-[10px] text-slate-400 font-bold uppercase px-2 select-none">Category:</span>
                    {CATEGORIES.map((cat) => (
                      <button
                        key={cat}
                        onClick={() => setActiveFilter(cat)}
                        className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-colors cursor-pointer ${activeFilter === cat ? 'bg-[#047c1f] border-[#047c1f] text-white shadow-sm' : 'bg-white hover:bg-slate-50 border-slate-200 text-slate-600'}`}
                      >
                        {cat}
                      </button>
                    ))}
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 pt-2 border-t border-slate-100">
                    <span className="text-[10px] text-slate-400 font-bold uppercase px-2 select-none">Region:</span>
                    {STATES.map((st) => (
                      <button
                        key={st}
                        onClick={() => setStateFilter(st)}
                        className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-colors cursor-pointer ${stateFilter === st ? 'bg-[#047c1f] border-[#047c1f] text-white shadow-sm' : 'bg-white hover:bg-slate-50 border-slate-200 text-slate-600'}`}
                      >
                        {st}
                      </button>
                    ))}
